def read_settings(filename):
	...
	cyclekey = file[14].split(':')[-1].strip()
	if cyclekey.lower() in ['true', '1', 't', 'y', 'yes', 'yeah', 'yup', 'certainly']:
		modelvars['cyclekey'] = True
	else:
		modelvars['cyclekey'] = False
	

	# Reading Transport variables
	cf5  = file[17].split('=')[-1].strip()
	cf6  = file[18].split('=')[-1].strip()
	cf7  = file[19].split('=')[-1].strip()
	cf8  = file[20].split('=')[-1].strip()
	cf9  = file[21].split('=')[-1].strip()
	cf10 = file[22].split('=')[-1].strip()
	cf11 = file[23].split('=')[-1].strip()
	cf12 = file[24].split('=')[-1].strip()
	cf13 = file[25].split('=')[-1].strip()
	cf14 = file[26].split('=')[-1].strip()
	cf15 = file[27].split('=')[-1].strip()
	cf16 = file[28].split('=')[-1].strip()
